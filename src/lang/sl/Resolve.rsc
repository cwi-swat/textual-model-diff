...
NameGraph resolve(Machine m) {
  env = ( s.id.name: s.id@location | State s <- m.states );
  g = ();
  for (s <- m.states) {
...
NameGraph resolve(transition(e, s), Scope scope, NameGraph g) {
  switch (s) {
    case r:ref(x): {
    ;
    }
...
NameGraph resolve(r:ref(x), Scope scope, NameGraph g) {
  g.uses += {r@location};
  g.refs += <r@location, scope.env[x]>;
  return g;
}

NameGraph resolve(r:ref(x, t), Scope scope, NameGraph g) {
  g.uses += {r@location};
  g.refs += <r@location, scope.env[x]>;
